...
For users to effectively use the app we just built, they need to always upload a transparent image with its background removed (else the use case of the app becomes useless 🫠). So I thought why not build another app that allows a user to remove the background of an image first before proceeding to add background colors? Well, the team that built CE.SDK also built the [background-removal.js](https://github.com/imgly/background-removal-js) library that allows you to remove backgrounds from images right in the browser environment with no additional costs or privacy concerns! The library used the Neural Network ([ONNX model](https://onnx.ai/)) and WASM files which are hosted by IMG.LY by default.
